...
public class SingleTableModel implements DataModel {
    ...
    // Each key is the time instance and its value are the vIDs that are alive.
    public HashMap<String, ArrayList<String>> getAllAliveVertices(String first, String last) {
        long tStart, tEnd, tDelta;

        HashMap<String, ArrayList<String>> vertices = new HashMap<>();
        // The "allVertices" key contains a list of all vIDs that are alive at some
        // point in [first, last]
        vertices.put("allVertices", new ArrayList<>());

        tStart = System.currentTimeMillis();
        FindIterable<Document> cursor = database.getCollection("dianode").find()
                .projection(Projections.include("_id.vid", "_id.start", "_id.end")).noCursorTimeout(true);
        ...
        for (Document row : cursor) {
            ...
            // Assumes correct intervals as input
            if (rowend <= Integer.parseInt(first) || Integer.parseInt(last) < rowstart)
                continue;

            String vid = id.getString("vid");
            // Only report values that are after both "first" and the diachronic node's
            // "rowstart"
            int start = Math.max(Integer.parseInt(first), rowstart);
            // Only report values that are before both "last" and the diachronic node's
            // "rowend"
            int end = Math.min(Integer.parseInt(last), rowend);
        ...
        // Remove duplicate vIDs from "allVertices"
        List<String> duplicates = vertices.get("allVertices");
    ...
    public Map<String, Map<String, Edge>> getAllEdgesOfVertex(String vid, String timestamp) {
        ...
        Document vertexRow = cursor.first();
    ...
    public List<SnapshotResult> getAvgVertexDegree(String first, String last) // Running example query range: [30,50]
    {
        ...
        for (String vertex : allVertices) {
            FindIterable<Document> cursor = database.getCollection("dianode").find(Filters.eq("_id.vid", vertex))
                    .projection(Projections.include("_id.start", "_id.end", "outgoing_edges")).noCursorTimeout(true);
    ...
    public HashMap<String, HashMap<String, Integer>> getDegreeDistribution(String first, String last) {
        ...
        FindIterable<Document> resultSetFuture = database.getCollection("dianode")
                .find(Filters.and(Filters.gte("_id.end", first), Filters.lt("_id.start", last)))
                .projection(Projections.include("_id.start", "_id.end", "outgoing_edges"))
                .noCursorTimeout(true);
        tEnd = System.nanoTime();
        tDelta = tEnd - tStart;
        ...
        for (Document row : resultSetFuture) // For each diachronic node
        {
            ...
            if (Integer.parseInt(rowend) < Integer.parseInt(first)) // That means that the diachronic node's "start" and
                                                                    // "end" time instances were BOTH before our query
                                                                    // instance
                continue;

            String rowstart = id.getString("start");

            int start = Math.max(Integer.parseInt(first), Integer.parseInt(rowstart)); // Only report values that are
                                                                                       // after both "first" and the
                                                                                       // diachronic node's "rowstart"
            int end = Math.min(Integer.parseInt(last), Integer.parseInt(rowend)); // Only report values that are before
                                                                                  // both "last" and the diachronic
                                                                                  // node's "rowend"

            Document edgesUDTList = (Document) row.get("outgoing_edges");
            Map<String, List<Edge>> outgoing_edges = SingleTableModel.convertToEdgeList(edgesUDTList); // Fetch all the
                                                                                                       // edges of the
                                                                                                       // diachronic
                                                                                                       // node
    ...
    public List<String> getOneHopNeighborhood(String vid, String first, String last) {
        ...
        for (String targetID : outgoing_edges.keySet()) {
            List<Edge> edges = outgoing_edges.get(targetID);
            for (Edge edge : edges)
                if ((Integer.parseInt(last) >= Integer.parseInt(edge.start)
    ...
    private void parseFirstSnapshot(String input, int snap_count) // Used to bulk load data instead of using the typical
                                                                  // methods
    {
        try {
            ...
            while ((line = file.readLine()) != null) {
                if (line.startsWith("mkdir") || line.startsWith("cd") || line.startsWith("time")
                        || line.startsWith("string") || line.startsWith("double") || line.startsWith("shutdown"))
                    continue;

                if (line.equals("graph 1 0") || line.equals("graph 1")) // As soon as we've reached the second snapshot,
                                                                        // stop
                    break;
                    ...
                    sVer.addOutgoingEdge(targetID, new Edge("testlabel", weight, targetID, "00000000",
                            DataModel.padWithZeros("" + snap_count)));
                    Vertex tVer = vertices.get(targetID);
                    tVer.addIncomingEdge(sourceID, new Edge("testlabel", weight, sourceID, "00000000",
                            DataModel.padWithZeros("" + snap_count)));
    ...
    public void parseInput(String input) {
        try {
            ...
            while ((line = file.readLine()) != null) {
                ...
                if (!passedFirstSnapshot || line.startsWith("mkdir") || line.startsWith("cd") || line.startsWith("time")
                        || line.startsWith("string") || line.startsWith("double") || line.startsWith("shutdown"))
                    continue;
                ...
                if (line.startsWith("graph")) {
                    ...
                    if (tokens.length == 4)
                        ...
                        color = getRandomString(4);
                    insertVertex(verID, name, DataModel.padWithZeros(curVersion),
                            DataModel.padWithZeros("" + snap_count), color);
                    verKcounter++;
                    ...
                    if (tokens.length == 4)
                        ...
                        weight = "" + Math.random();
                    insertEdge(sourceID, targetID, DataModel.padWithZeros(curVersion),
                            DataModel.padWithZeros("" + snap_count), getRandomString(3), weight);
                    edgeKcounter++;
